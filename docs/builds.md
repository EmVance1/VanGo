...
- **settings**: the following are broad toolchain agnostic settings that translate to various compiler and linker options. A * indicates a global default if applicable.
    ...
    * `no-rtti`: (C++) disable RTTI (`true|false*`)
    * `no-except`: (C++) disable exceptions (`true|false*`)
